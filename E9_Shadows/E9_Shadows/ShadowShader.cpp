...
void ShadowShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* heightMap, ID3D11ShaderResourceView*depthMap, ID3D11ShaderResourceView* depthMap2, Light* light, Light* light2)
{
	...
	XMMATRIX tLightViewMatrix1 = XMMatrixTranspose(light->getViewMatrix());
	XMMATRIX tLightProjectionMatrix1 = XMMatrixTranspose(light->getOrthoMatrix());
	...
	dataPtr->lightView = tLightViewMatrix1;
	dataPtr->lightProjection = tLightProjectionMatrix1;
	dataPtr->lightView2 = tLightViewMatrix2;
	dataPtr->lightProjection2 = tLightProjectionMatrix2;
	deviceContext->Unmap(matrixBuffer, 0);
	deviceContext->VSSetConstantBuffers(0, 1, &matrixBuffer);

	// Map and set the light buffer
	deviceContext->Map(lightBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	lightPtr = (LightBufferType*)mappedResource.pData;
	lightPtr->ambient = light->getAmbientColour();
	lightPtr->diffuse = light->getDiffuseColour();
	lightPtr->direction = light->getDirection();
	lightPtr->padding = 0.0f; // Padding if needed for alignment

	// Set second light's properties in the buffer
